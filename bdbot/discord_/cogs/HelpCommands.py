...
class HelpCommands(commands.Cog):
    ...
    async def general(self, inter: discord.Interaction):
        ...
        return await send_embed(
            inter, [get_general_help(self.help_dict[HELP_GENERAL_NAME])]
        )
    ...
    async def gocomics(self, inter: discord.Interaction):
        """Gocomics help"""
        await send_embed(inter, Gocomics.get_website_help_embed())

    @help_group.command()
    async def comicskingdom(self, inter: discord.Interaction):
        """Comics Kingdom help"""
        await send_embed(inter, ComicsKingdom.get_website_help_embed())

    @help_group.command()
    async def webtoons(self, inter: discord.Interaction):
        """Webtoons help"""
        await send_embed(inter, Webtoons.get_website_help_embed())
