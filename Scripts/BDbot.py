...
class BDBot(commands.Cog):
  ...
  @commands.command()
  async def invite(self,client,ctx): # Creates a Oauth2 link to share the bot
    inv = discord.utils.oauth_url(client)
  ...
  def create_Embed(comic_details=None):
    if(comic_details!=None):
      ...
      embed=discord.Embed(title=f"{comic_name} for {day}/{month}/{year}", url = url)
      ...
      if(comic_details["alt"]!=None): # If there is alt text (Text when you hover your mouse on the image)
        ...
        embed.set_field(name="Alt text",value=alt)
  ...
  async def send_comic_embed(self, ctx, comic_details):
    embed = self.create_Embed(comic_details) # Creates the embed
