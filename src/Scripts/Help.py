...
class Help(commands.Cog):
    ...
    async def general(self, ctx: commands.Context):
        ...
        if discord_utils.HELP_EMBED is None:
            ...
            embed.add_field(name="Request",
                            value="Have a request for the bot? Post your request at https://github.com/BBArikL/BDBot/"
                                  "issues/new?assignees=&labels=enhancement&template=comic-request.md&title="
                                  "New+Comic+request for maximum visibility or use `/request` to leave a message to"
                                  " the developer!")
    ...
    async def hourly(self, ctx):
        ...
        if discord_utils.HOURLY_EMBED is None:
            ...
            embed = discord.Embed(title="Daily commands!",
                                  description="Date and hour are optional arguments that can specify when the the bot "
                                              "should send the comic. A date should be one of the seven days of the "
                                              "week and the hour a number representing the time in a 24h clock in UTC "
                                              "time. If not specified, defaults to the current time in UTC.")
            ...
            embed.add_field(name="Add",
                            value="Use `/<name_of_comic> add <date> <hour>` to add the comic to the daily list of the "
                                  "channel.")
            embed.add_field(name="Add all",
                            value="Use `/add_all <date> <hour>` to add all the comics to a specific day of the week "
                                  "and hour.")
            ...
            embed.add_field(name="Remove all",
            ...
            embed.add_field(name="Set role mention",
                            value="Use `/set_role @<role>` to add a role to mention for comics posts. To remove, "
                                  "use `/remove_role`.")
            embed.add_field(name="Mange role mention",
    ...
    async def new(self, ctx):
        ...
        if discord_utils.NEW_EMBED is None:
            embed = discord.Embed(title="New features",
            ...
            embed.add_field(name="Enable/Disable post announcement",
            ...
            embed.add_field(name="Status",
                            value="Get the status of the bot with these 3 new commands: `/ping`, `/uptime` and"
                                  " `/status`.")
            embed.add_field(name="Delete requests",
                            value="Want to delete previous requests that that you sent? Use `/request_delete` to"
                                  " delete all your previous requests.")
            embed.add_field(name="FAQ",
                            value="Have any question on the bot? This FAQ (`/faq`) might have the response you need!")
            embed.add_field(name="Optimizations",
    ...
    async def faq(self, ctx):
        ...
        if discord_utils.FAQ_EMBED is None:
            ...
            embed.add_field(name="How can I support the project?",
            ...
            embed.add_field(name="What information is collected by using this bot?",
                            value="No personal information is used by this bot if you only use it to read comics. The "
                                  ...
                                  "requests sent), use `/request_delete`.",
                            inline=False)
