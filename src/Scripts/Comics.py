...
from src.discord_utils import get_possible_hours, parameters_interpreter, get_possible_days, get_possible_years
...
class Comic(commands.Cog):
    ...
    @app_commands.choices(hour=get_possible_hours(), day=get_possible_days(), year=get_possible_years())
    async def garfield(self, inter: discord.Interaction, action: Action,
                       date: Date = None, hour: int = None, day: int = 1,
                       month: Month = Month.February, year: int = 1950):
        """Garfield"""
        comic_name = 'Garfield'

        # Interprets the parameters given by the user
        await parameters_interpreter(inter, get_strip_details(comic_name), action=action, date=date, hour=hour, day=day,
                                     month=month, year=year)

    @app_commands.command()
    @app_commands.choices(hour=get_possible_hours(), day=get_possible_days(), year=get_possible_years())
    async def garfield_classics(self, inter: discord.Interaction, action: Action, date: Date = None, hour: int = None,
                                day: int = 1,
                                month: Month = Month.February, year: int = 1950):
        """Garfield classics"""
    ...
    async def random(self, inter: discord.Interaction, action: Action,
        ...
        await parameters_interpreter(inter, get_strip_details(random.choice(
            list(strip_details.keys()))), action=action, date=date, hour=hour, day=day, month=month, year=year)
    ...
    async def all(self, inter: discord.Interaction, action: Action,
        ...
        for com in strp:
            # Interprets the parameters given by the user
            await parameters_interpreter(inter, get_strip_details(com), action=action, date=date, hour=hour, day=day,
                                         month=month, year=year)
